package com.example.practice_fitness.fitness


import com.example.practice_fitness.R
...
import kotlinx.android.synthetic.main.fitness_hi.*
import java.io.File
...
class RectOverlay constructor(context: Context?, attributeSet: AttributeSet?) :
    ...
    private val STROKE_WIDTH = 7f // has to be float
...
class Fitness_Hi : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.fitness_hi)
...
//        camera_capture_button.setOnClickListener { takePhoto() }
    ...
    override fun onRequestPermissionsResult(requestCode: Int, permissions: Array<String>, grantResults: IntArray) {
        super.onRequestPermissionsResult(requestCode, permissions, grantResults) //오류 해결
    ...
    private fun onTextFound(pose: Pose)  {
        try {
            ...
            val leftPinky = pose.getPoseLandmark(PoseLandmark.LEFT_PINKY) // 왼 새끼손가락
            val rightPinky = pose.getPoseLandmark(PoseLandmark.RIGHT_PINKY) // 오른 새끼손가락
            val leftIndex = pose.getPoseLandmark(PoseLandmark.LEFT_INDEX)
            val rightIndex = pose.getPoseLandmark(PoseLandmark.RIGHT_INDEX)
            val leftThumb = pose.getPoseLandmark(PoseLandmark.LEFT_THUMB)
            val rightThumb = pose.getPoseLandmark(PoseLandmark.RIGHT_THUMB)
            val leftHeel = pose.getPoseLandmark(PoseLandmark.LEFT_HEEL) // 왼 뒷꿈치
            val rightHeel = pose.getPoseLandmark(PoseLandmark.RIGHT_HEEL) // 오른 뒷꿈치
            ...
            //Calculate whether the hand exceeds the shoulder = 손이 어깨를 초과하는지
            val yRightHand = rightWrist!!.position.y - rightShoulder!!.position.y
            val yLeftHand = leftWrist!!.position.y - leftShoulder!!.position.y
            //Calculate whether the distance between the shoulder and the foot is the same width = 어깨가 발 사이의 거리가 같은 너비인지 계산
            ...
            val ratio = footDistance/shoulderDistance //ratio = 비율
...
/*            if( leftHip != null && leftKnee != null  && leftAnkle != null){
            ...
            //Toast.makeText(this@Fitness_Hi, "Errore", Toast.LENGTH_SHORT).show()
